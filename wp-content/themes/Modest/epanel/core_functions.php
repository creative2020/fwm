				...
				<div class="epanel-box">
					...
					<div class="box-content">
						...
						<?php if ($value['type'] == "text") { ?>
							...
							<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo esc_attr( stripslashes(get_option( $value['id']) ) ) ; } else { echo esc_attr(stripslashes($value['std'])); } ?>" />
							
						<?php } elseif ($value['type'] == "textlimit") { ?>
						
							<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="text" maxlength="<?php echo $value['max']; ?>" size="<?php echo $value['max']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo esc_attr( stripslashes(get_option( $value['id'] ) ) ); } else { echo esc_attr(stripslashes($value['std'])); } ?>" />
						...
						<?php } elseif ($value['type'] == "textcolorpopup") { ?>
							...
							<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" class="colorpopup" type="text" value="<?php if ( get_option( $value['id'] ) != "") { echo esc_attr(get_option( $value['id'] )); } else { echo esc_attr($value['std']); } ?>" />
							
						<?php } elseif ($value['type'] == "textarea") { ?>
						
							<textarea name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>"><?php if ( get_option( $value['id'] ) != "") { echo esc_textarea(stripslashes(get_option( $value['id'] ))); } else { echo esc_textarea(stripslashes($value['std'])); } ?></textarea>
							
						<?php } elseif ($value['type'] == "upload") { ?>
								
								<input id="<?php echo $value['id']; ?>" class="uploadfield" type="text" size="90" name="<?php echo $value['id']; ?>" value="<?php echo esc_url(get_option($value['id'])); ?>" />
			...
			foreach ($options as $value) {
				if( isset( $value['id'] ) ) { 
					if( isset( $_REQUEST[ $value['id'] ] ) ) {
						if ( $value['type'] == 'text' || $value['type'] == 'textlimit' ) 
							esc_html( update_option( $value['id'], $_REQUEST[$value['id']] ) );
						elseif ( $value['type'] == 'upload' || $value['type'] == 'select' )
							update_option( $value['id'], wp_strip_all_tags( $_REQUEST[$value['id']] ) );
...
#filter main options ( it happens when using get_option() in the code )
function et_stripslash_main_options(){
	global $shortname;
	$options = apply_filters( 'et_stripslash_options', array( '_468_adsense', '_integration_single_top', '_integration_single_bottom', '_seo_home_descriptiontext', '_seo_home_separate' ) );
	foreach ( $options as $option_name ){
		add_filter( 'option_' . $shortname . $option_name, 'et_getoption_stripslashes' );
	}
}

function et_getoption_stripslashes( $option ){
	return stripslashes( $option );
}

et_stripslash_main_options(); ?>